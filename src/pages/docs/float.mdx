...
`.float-right`を使用して、要素をそのコンテナの右側にフロートさせます。
...
`.float-right`を使用して、要素をそのコンテナの左側にフロートさせます。
...
`.float-none`を使用して、要素に適用されているフロートをリセットします。これは、floatプロパティのデフォルト値です。
...
特定のブレークポイントで要素のフロートを制御するには、既存のフロートユーティリティクラスに`{screen}:`プレフィックスを追加します。
たとえば、`md:float-left`を使用して、中程度の画面サイズ以上でのみ`float-left`ユーティリティを適用します。
